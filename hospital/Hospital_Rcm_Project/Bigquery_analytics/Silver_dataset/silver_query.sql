...
CREATE TABLE IF NOT EXISTS `hospital-project1-494408


.silver_dataset.departments` (
    Dept_Id STRING,
    SRC_Dept_Id STRING,
    Name STRING,
    datasource STRING,
...
TRUNCATE TABLE `hospital-project1-494408


.silver_dataset.departments`;

-- 3. full load by Inserting merged Data 
INSERT INTO `hospital-project1-494408
...
FROM (
    SELECT DISTINCT *, 'hosa' AS datasource FROM `hospital-project1-494408


.bronze_dataset.departments_ha`
    UNION ALL
    SELECT DISTINCT *, 'hosb' AS datasource FROM `hospital-project1-494408


.bronze_dataset.departments_hb`
);
...
.bronze_dataset.providers_ha`
    UNION ALL
    SELECT DISTINCT *, 'hosb' AS datasource FROM `hospital-project1-494408


.bronze_dataset.providers_hb`
);
...
CREATE OR REPLACE TABLE `hospital-project1-494408


.silver_dataset.quality_checks` AS
SELECT DISTINCT 
...
FROM (
    SELECT DISTINCT 
        ...
        ModifiedDate,
        'hosa' AS datasource
    FROM `hospital-project1-494408


.bronze_dataset.patients_ha`
...
.bronze_dataset.patients_hb`
);

-- 3. Apply SCD Type 2 Logic with MERGE
MERGE INTO `hospital-project1-494408


.silver_dataset.patients` AS target
USING `hospital-project1-494408


.silver_dataset.quality_checks` AS source
...
DROP TABLE IF EXISTS `hospital-project1-494408


.silver_dataset.quality_checks`;
...
.bronze_dataset.transactions_ha`
    UNION ALL
    SELECT DISTINCT *, 'hosb' AS datasource FROM `hospital-project1-494408


.bronze_dataset.transactions_hb`
);
...
.silver_dataset.transactions` AS target
USING `hospital-project1-494408
...
.silver_dataset.quality_checks_encounters` AS
SELECT DISTINCT 
...
.silver_dataset.encounters` AS target
USING `hospital-project1-494408


.silver_dataset.quality_checks_encounters` AS source
...
.silver_dataset.quality_checks_encounters`;
...
.silver_dataset.quality_checks_claims` AS
SELECT 
...
.silver_dataset.claims` AS target
USING `hospital-project1-494408


.silver_dataset.quality_checks_claims` AS source
...
.silver_dataset.quality_checks_claims`;
...
.silver_dataset.quality_checks_cpt_codes` AS
SELECT 
...
.silver_dataset.cpt_codes` AS target
USING `hospital-project1-494408


.silver_dataset.quality_checks_cpt_codes` AS source
...
.silver_dataset.quality_checks_cpt_codes`;
